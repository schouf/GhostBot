...
'''
    ...
    prompt = f"""
You are a Master Cinematographer and Archival Researcher for a True Crime YouTube channel.
...
RULE 1: 'search_query' (For Wikipedia Image Search - REAL EVIDENCE)
- MUST be strictly 2 to 4 keywords. Wikipedia fails with long sentences.
- Use EXACT historical names, years, objects, or locations. 
- NEVER use adjectives like "creepy", "mysterious", or "scary".
- Good Example: "Max Headroom incident" or "Somerton Man"
- Bad Example: "A creepy mysterious man in a mask"

RULE 2: 'ai_prompt' (For FLUX.1 High-End AI Generation)
- Use this formula: [Subject/Action] + [Setting] + [Lighting] + cinematic 35mm photography, 8k resolution, vertical composition
- NEVER ask for text, numbers, letters, documents with writing, or signs. AI text looks like gibberish.
...
def fetch_archive_image(prompt, filename):
    """Layer 1: Dual-Archive Search (Wikipedia API + Internet Archive)"""
    print(f"🏛️ [1/4] Public Archives Search: {prompt[:40]}...")
    
    # 1A. Wikipedia API Attempt
    ...
    try:
        ...
        pass # Silently fallback to Internet Archive

    # 1B. Internet Archive Attempt
    ...
    try:
        ...
        for doc in docs:
            ...
            if identifier:
                # Construct the direct download URL for the JPEG
                img_url = f"https://archive.org/download/{identifier}/{identifier}.jpg"
                img_data = requests.get(img_url, headers=headers, timeout=15).content
                if len(img_data) > 1000: # Ensure it's not a tiny error page
                    with open(filename, "wb") as f:
                        f.write(img_data)
...
def generate_youtube_metadata(full_script_text, sota_models):
    ...
    d_prompt = f"Write a compelling 3-sentence description for YouTube Short titled: '{title}'. No hashtags."
    ...
    tag_prompt = f"Title: '{title}'. Description: '{description}'. Provide exactly 8 highly-searched SEO tags as a comma-separated list. No hashtags."
