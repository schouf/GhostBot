...
def ask_llm(system_instruction, prompt, sota_models):
    strict_prompt = prompt + "\n\nRÈGLE CRITIQUE: Retourne UNIQUEMENT le texte exact demandé. Pas de texte d'introduction ou de conversation."
...
def generate_viral_script(fallback_sota_models):
    ...
    content_pool = [
        "Disparitions inexpliquées et bizarres", "Braquages et cambriolages impossibles",
        "Personnes ayant simulé leur propre mort", "Vrais glitches dans la matrice",
        "Artefacts historiques qui n'auraient pas dû exister", "Piratages de diffusion TV et radio"
    ]
    ...
    avoid_instruction = f"CRITIQUE: Ne PAS écrire sur ces affaires déjà traitées:\n{past_topics}\n" if past_topics else ""

    json_template = '''
{
    "case_name": "L'Homme de Somerton",
    "recommended_voice_model": "Charon",
    "lines": [
        {
            "style_instruction": "Chuchotement effrayé et retenu.",
            "acting_text": "Il est entré dans la pièce... <break time='1.5s'/> et a <emphasis>disparu</emphasis>.",
            "clean_text": "Il est entré dans la pièce et a disparu."
        }
    ]
}
'''
    ...
    json_template = '''
{
    "visuals": [
        {
            "search_query": "homme Somerton plage 1948",
            "ai_prompt": "Un téléviseur cathodique allumé dans un salon des années 70 plongé dans l'obscurité totale, parasites lumineux intenses, brouillard volumétrique inquiétant, photographie cinématographique 35mm, ombres profondes, très détaillé, composition verticale"
        }
    ]
}
'''
    ...
    prompt = f"""
Tu es un Directeur de la Photographie et Chercheur en Archives pour une chaîne YouTube True Crime française.

Associe des directions visuelles séquentielles au script de voix off ci-dessous.

SCRIPT:

"{full_script_text}"

Nous avons besoin de EXACTEMENT {required_images} transitions visuelles.

RÈGLE 1: 'search_query' (Pour la recherche d'images Pexels - stock de qualité)

- DOIT être strictement 2 à 4 mots-clés en anglais (Pexels est en anglais).
- Utilise des noms historiques exacts, années, objets ou lieux.
- NE JAMAIS utiliser des adjectifs comme "creepy", "mysterious" ou "scary".
- Bon exemple: "diamond heist vault" ou "missing person forest"
- Mauvais exemple: "A creepy mysterious man in a mask"

RÈGLE 2: 'ai_prompt' (Pour la génération IA FLUX.1 haute qualité)

- Utilise cette formule: [Sujet/Action] + [Décor] + [Éclairage] + photographie cinématographique 35mm, résolution 8k, composition verticale
- NE JAMAIS demander du texte, des chiffres, des lettres, des documents avec de l'écriture ou des panneaux.

Fournis EXACTEMENT {required_images} éléments. Retourne UNIQUEMENT du JSON valide correspondant à ce format:
...
def generate_youtube_metadata(full_script_text, sota_models):
    ...
    sys_prompt = "Tu es un expert SEO YouTube Shorts francophone d'élite. Tu produis UNIQUEMENT le texte exact demandé."

    t_prompt = f"Lis ce script et écris UN titre YouTube Shorts accrocheur en FRANÇAIS (moins de 60 caractères). Pas de guillemets ni hashtags.\nScript: {full_script_text}"
    title = ask_llm(sys_prompt, t_prompt, sota_models).strip('"').replace("'", "")

    if not title or len(title) > 80: title = "Ils ont trouvé QUOI ?"

    d_prompt = f"Écris une description captivante de 3 phrases en FRANÇAIS pour ce YouTube Short intitulé: '{title}'. Pas de hashtags."
    description = ask_llm(sys_prompt, d_prompt, sota_models)

    if not description: description = "Un mystère inexpliqué qui va vous laisser sans voix."

    tag_prompt = f"Titre: '{title}'. Description: '{description}'. Fournis exactement 8 tags SEO très recherchés en français sous forme de liste séparée par des virgules. Pas de hashtags."
    tags_str = ask_llm(sys_prompt, tag_prompt, sota_models)

    tags = [t.strip().replace("#", "") for t in tags_str.split(',')] if tags_str else ["mystère", "shorts", "creepy", "inexpliqué", "truecrime", "histoire", "crime", "france"]

    return {"title": f"{title} #shorts #mystère", "description": description, "tags": tags}

def generate_platform_captions(yt_metadata, platform, sota_models):
    sys_prompt = f"Tu es un expert Social Media Manager {platform} francophone d'élite. Tu produis UNIQUEMENT le texte final de la légende."

    if platform == "Instagram":
        prompt = f"Convertis en légende Instagram Reels virale en FRANÇAIS.\nTitre: {yt_metadata['title']}\nDescription: {yt_metadata['description']}\nEXIGENCES: Accroche forte, call-to-action, 6 hashtags, emojis."
    else:
        prompt = f"Convertis en légende Facebook Reels engageante en FRANÇAIS.\nTitre: {yt_metadata['title']}\nDescription: {yt_metadata['description']}\nEXIGENCES: Ton conversationnel, pose une question, 3 hashtags."
...
def main_pipeline():
    ...
    # THE KILL SWITCH: Prevent 7-Minute Hallucinations
    if len(script.get("lines", [])) > 12:
        print("⚠️ Script exceeded 12 lines! Truncating to prevent API bans and 7-minute videos...")
        script["lines"] = script["lines"][:12]
